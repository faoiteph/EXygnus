...
use DeFaoite\Core\Exceptions\Handler;
use DeFaoite\Core\Facades\ElasticSearch;
use DeFaoite\Core\View\Compilers\BladeCompiler;
...
class CoreServiceProvider extends ServiceProvider
{
    ...
    protected function registerOverrides(): void
    {
        $this->app->extend(
            UpCommand::class,
            fn () => new \DeFaoite\Core\Console\Commands\UpCommand
        );

        $this->app->extend(
            DownCommand::class,
            fn () => new \DeFaoite\Core\Console\Commands\DownCommand
        );
        ...
        $this->app->bind(
            ...
            fn ($app) => new \DeFaoite\Core\Http\Middleware\PreventRequestsDuringMaintenance($app)
        );
